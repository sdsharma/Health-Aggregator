...
var usersFilePath = path.join(__dirname, '/views/macrosdata.json');
 app.get("/", function(req, res) {
    ...
    mfp.fetchDateRange('superdudeb', getDateTime(7), getDateTime(0), 'all', function(data){
      ...
      var stepsPoints = Math.floor((jwdata[days-8].m_steps * 50)/10000);
      var weightPoints = (Math.ceil(jwdata[days-9].weight * 2.2) - Math.ceil(jwdata[days-8].weight * 2.2)) * 50;
      if( weightPoints < 0 )
      {
          weightPoints = 0;
      }
      var caloriesBurnedPoints = Math.floor(jwdata[days-8].m_calories * 0.1);
      ...
      var caloriesPoints = Math.floor((data.data[data.data.length - 2].calories - data.data[data.data.length - 1].calories) * 0.25);
      if( caloriesPoints < 0 )
      {
          caloriesPoints = 0;
      }
      var activePoints =  (jwdata[days-8].m_active_time/3600).toFixed(2)*25;
      var totalPoints = stepsPoints + weightPoints + caloriesBurnedPoints + inactivePoints + sleepPoints + caloriesPoints + activePoints;
      // console.log(jwdata[day-7]);
      res.render('index.ejs', {totalPoint: totalPoints, stepPoint: stepsPoints, weightPoint: weightPoints, caloriesBurnedPoint: caloriesBurnedPoints, 
...
function getMealsToday(){
  var url = "https://www.myfitnesspal.com/reports/printable_diary/superdudeb?from=" + getDateTime(1) + "&to=" + getDateTime(1);
